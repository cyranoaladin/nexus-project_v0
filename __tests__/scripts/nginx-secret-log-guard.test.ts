...
describe('nginx bearer path logging', () => {
  ...
  const localNginx = readFileSync('nginx/nginx.local.conf', 'utf8');
  ...
  it('redacts signed bilan paths and their referrers before access logging', () => {
    ...
    expect(nginx).toContain('~^/devis/[^/]+ /devis/[REDACTED]');
    expect(nginx).toContain('~^/api/quotes/public/[^/]+ /api/quotes/public/[REDACTED]');
  });

  it('redacts family quote bearer paths and referrers in every canonical log format', () => {
    for (const config of [nginx, localNginx, productionInclude]) {
      expect(config).toContain('~^/devis/[^/]+ /devis/[REDACTED]');
      expect(config).toContain('~^/api/quotes/public/[^/]+ /api/quotes/public/[REDACTED]');
      expect(config).toContain('(?:devis|api/quotes/public)/ [REDACTED]');
    }
  });
  ...
  it('suppresses the raw built-in error log inside bearer-bearing locations', () => {
    for (const location of bearerPaths) {
      for (const config of [nginx, localNginx, sensitiveLocations]) {
