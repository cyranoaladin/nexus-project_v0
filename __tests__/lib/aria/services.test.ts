...
describe('lib/aria/services', () => {
  ...
  it('llm_service.generate_response returns JSON on 200', async () => {
    global.fetch = jest.fn().mockResolvedValue({ ok: true, json: async () => ({ response: 'ok' }) });
    const res = await llm_service.generate_response({ contexte_eleve: {}, requete_actuelle: 'q', requete_type: 't' } as any);
  ...
  it('pdf_generator_service.generate_pdf handles non-OK by throwing (wrapped error)', async () => {
    ...
    await expect(
      pdf_generator_service.generate_pdf({ contenu: 'x', type_document: 'TD', matiere: 'NSI', nom_fichier: 'f', nom_eleve: 'n' } as any)
  ...
  it('rag_service.ingest throws on network error', async () => {
    ...
    await expect(
      rag_service.ingest({ contenu: 'abc', metadata: {} })
    ).rejects.toThrow(/Erreur de communication/);
  });
});
