...
describe('AriaOrchestrator branch coverage', () => {
  ...
  it('generates PDF when query requests it and LLM returns latex', async () => {
    (prisma as any).student = { findUnique: jest.fn().mockResolvedValue({ id: 's1', user: { firstName: 'A', lastName: 'B' }, parent: { user: { firstName: 'P', lastName: 'Q' } } }) };
    (prisma as any).ariaMessage = { findMany: jest.fn().mockResolvedValue([]), createMany: jest.fn().mockResolvedValue({ count: 2 }) };
    (prisma as any).ariaConversation = { findFirst: jest.fn().mockResolvedValue(null), create: jest.fn().mockResolvedValue({ id: 'c1' }) };
    (llm_service as any).generate_response = jest.fn().mockResolvedValue({ response: 'ok', contenu_latex: 'latex' });
    (pdf_generator_service as any).generate_pdf = jest.fn().mockResolvedValue({ url: 'http://doc.pdf' });
  ...
  it('PDF fallback kicks in when first compile fails and second succeeds', async () => {
    (prisma as any).student = { findUnique: jest.fn().mockResolvedValue({ id: 's1', user: { firstName: 'A', lastName: 'B' } }) };
    (prisma as any).ariaMessage = { findMany: jest.fn().mockResolvedValue([]), createMany: jest.fn().mockResolvedValue({ count: 2 }) };
    (prisma as any).ariaConversation = { findFirst: jest.fn().mockResolvedValue(null), create: jest.fn().mockResolvedValue({ id: 'c1' }) };
    (llm_service as any).generate_response = jest.fn().mockResolvedValue({ response: 'texte suffisant pour fallback'.repeat(20), contenu_latex: 'bad\\write18' });
  ...
  it('uses existing LaTeX (with \\documentclass) without wrapping and compiles once', async () => {
    ...
    (llm_service as any).generate_response = jest.fn().mockResolvedValue({ response: 'ok', contenu_latex: latex });
    const gen = (pdf_generator_service as any).generate_pdf = jest.fn().mockResolvedValue({ url: 'http://doc2.pdf' });
    const orch = new AriaOrchestrator('s1', 'p1');
  ...
  it('decision_hints switches to REMEDIATION when multiple weaknesses', async () => {
    (prisma as any).student = { findUnique: jest.fn().mockResolvedValue({ id: 's1', user: { firstName: 'A', lastName: 'B' } }) };
    (prisma as any).ariaMessage = { findMany: jest.fn().mockResolvedValue([]), createMany: jest.fn().mockResolvedValue({ count: 2 }) };
    (prisma as any).ariaConversation = { findFirst: jest.fn().mockResolvedValue({ id: 'c1' }), create: jest.fn() };
    ...
    const llmSpy = (llm_service as any).generate_response = jest.fn().mockResolvedValue({ response: 'ok' });
    const orch = new AriaOrchestrator('s1', 'p1');
  ...
  it('throws when profile missing', async () => {
    ...
    (prisma as any).ariaMessage = { findMany: jest.fn().mockResolvedValue([]), createMany: jest.fn() };
    (prisma as any).ariaConversation = { findFirst: jest.fn(), create: jest.fn() };

    const orch = new AriaOrchestrator('sX', 'pX');
    await expect(orch.handleQuery('hello', 'NSI' as any)).rejects.toThrow(/Impossible de charger le profil/);
  });
  ...
  it('does not call pdf service if query asks pdf but no latex provided by LLM', async () => {
    ...
    (prisma as any).ariaMessage = { findMany: jest.fn().mockResolvedValue([]), createMany: jest.fn().mockResolvedValue({ count: 2 }) };
    (prisma as any).ariaConversation = { findFirst: jest.fn().mockResolvedValue({ id: 'c1' }), create: jest.fn() };
    (llm_service as any).generate_response = jest.fn().mockResolvedValue({ response: 'réponse textuelle seulement', contenu_latex: undefined });
