...
describe('Repères tarifaires == canonical offers (no second source)', () => {
  test('brevetMois == brevet-maths monthly_display', () => {
    expect(rep.brevetMois).toBe(`a partir de ${findOffer('brevet-maths').monthly_display} TND / mois`);
  });

  test('secondeMois == 2nde-maths monthly_display', () => {
    expect(rep.secondeMois).toBe(`a partir de ${findOffer('2nde-maths').monthly_display} TND / mois`);
  });

  test('premiereSimpleMois == 1re-eaf monthly_display', () => {
    expect(rep.premiereSimpleMois).toBe(`a partir de ${findOffer('1re-eaf').monthly_display} TND / mois`);
  });

  test('premiereDuoMois == 1re-double-secu monthly_display', () => {
    expect(rep.premiereDuoMois).toBe(`a partir de ${findOffer('1re-double-secu').monthly_display} TND / mois`);
  });

  test('terminaleSimpleMois == term-spe-simple monthly_display', () => {
    expect(rep.terminaleSimpleMois).toBe(`a partir de ${findOffer('term-spe-simple').monthly_display} TND / mois`);
  });
  ...
  test('plateformeAn == plateforme-autonomie price_annual', () => {
    expect(rep.plateformeAn).toBe(`a partir de ${findOffer('plateforme-autonomie').price_annual} TND / an`);
  });

  test('stagesBase == min(stage_formats.price_per_student)', () => {
    const min = Math.min(...stages.map((f) => f.price_per_student));
    expect(rep.stagesBase).toBe(`des ${min} TND`);
  });

  test('parrainage == rules.discounts range', () => {
    const rules = data.rules.discounts;
    expect(rep.parrainage).toBe(`${rules.parrainage_min_tnd} a ${rules.parrainage_max_tnd} TND`);
  });
});
