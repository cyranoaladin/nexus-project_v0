...
import { GET } from '@/app/api/parent/children/[studentId]/aria/workshops/route';
import { listAriaWorkshopsForParent } from '@/lib/aria/application/workshop/list-workshops-for-parent';
import { AriaError } from '@/lib/aria/errors';

jest.mock('@/auth', () => ({ auth: jest.fn() }));
jest.mock('@/lib/aria/application/workshop/list-workshops-for-parent', () => ({
  listAriaWorkshopsForParent: jest.fn(),
}));
...
function request(courseKey?: string) {
  const url = courseKey
    ? `http://localhost/api/parent/children/student-1/aria/workshops?courseKey=${courseKey}`
    : 'http://localhost/api/parent/children/student-1/aria/workshops';
...
describe('GET /api/parent/children/[studentId]/aria/workshops', () => {
  ...
  it('returns 401 when the session role is not PARENT', async () => {
    ...
    expect(listAriaWorkshopsForParent).not.toHaveBeenCalled();
  });
  ...
  it('uses only the authenticated actor and the URL studentId, and returns the real workshop feed', async () => {
    (auth as jest.Mock).mockResolvedValueOnce({ user: { role: 'PARENT', id: 'parent-1' } });
    (listAriaWorkshopsForParent as jest.Mock).mockResolvedValueOnce([]);
    const response = await GET(request('eds-maths-premiere') as never, context('student-42'));
    expect(response.status).toBe(200);
    expect(listAriaWorkshopsForParent).toHaveBeenCalledWith({
  ...
  it('maps an authorization failure to its stable public denial (e.g. a different family\'s child)', async () => {
    (auth as jest.Mock).mockResolvedValueOnce({ user: { role: 'PARENT', id: 'parent-1' } });
    (listAriaWorkshopsForParent as jest.Mock).mockRejectedValueOnce(
