...
describe('Admin Analytics - recent activities and formatting branches', () => {
  ...
  it('formats data with default amount=0 and maps activity actions for COMPLETED/SCHEDULED/CANCELLED/default', async () => {
    const now = new Date();
    (prisma as any).payment.groupBy = jest
      .fn()
      .mockResolvedValue([{ createdAt: now, _sum: { amount: undefined }, _count: { id: 1 } }]);
    (prisma as any).user.groupBy = jest
      .fn()
      .mockResolvedValue([{ createdAt: now, role: 'ELEVE', _count: { id: 2 } }]);
    (prisma as any).session.groupBy = jest
      .fn()
      .mockResolvedValue([{ scheduledAt: now, status: 'COMPLETED', _count: { id: 3 } }]);
    (prisma as any).subscription.groupBy = jest
      .fn()
      .mockResolvedValue([{ createdAt: now, status: 'ACTIVE', _count: { id: 4 } }]);
    (prisma as any).creditTransaction.groupBy = jest
      .fn()
      .mockResolvedValue([
    ...
    const res = await GET(
      new NextRequest('http://localhost/api/admin/analytics?period=month&type=all')
    );
    ...
    const actions = json.recentActivities.reduce(
      (set: Set<string>, a: any) => set.add(a.action),
      new Set<string>()
    );
