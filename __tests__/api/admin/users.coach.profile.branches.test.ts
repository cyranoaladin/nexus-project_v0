...
describe('Admin Users - COACH profile create/update branches', () => {
  ...
  it('POST creates COACH with profileData mapping (lines 165-172)', async () => {
    ...
    (prisma as any).user.create = jest.fn().mockImplementation(({ data }: any) => Promise.resolve({ id: 'u1', ...data, coachProfile: { pseudonym: data.coachProfile.create.pseudonym } }));
  ...
  it('POST creates COACH with empty profileData using defaults', async () => {
    (prisma as any).user.findUnique = jest.fn().mockResolvedValue(null);
    (prisma as any).user.create = jest.fn().mockImplementation(({ data }: any) => Promise.resolve({ id: 'u2', ...data, coachProfile: { pseudonym: data.coachProfile.create.pseudonym } }));
    const payload = {
    ...
    const { POST } = require('@/app/api/admin/users/route');
    const req = new NextRequest('http://localhost/api/admin/users', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) } as any);
    const res = await POST(req);
    expect(res.status).toBe(200);
    const call = (prisma as any).user.create.mock.calls[0][0];
  ...
  it('PUT updates COACH with upsert mapping (lines 235-242)', async () => {
    (prisma as any).user.update = jest.fn().mockImplementation(({ data }: any) => Promise.resolve({ id: 'u1', ...data, coachProfile: {} }));
