...
jest.mock('next-auth', () => ({
  getServerSession: jest.fn().mockResolvedValue({ user: { id: 'u1', role: 'PARENT', studentId: 's1', parentId: 'p1' } }),
}));
...
describe('API /api/aria/chat branches', () => {
  ...
  it('429 when rate-limit exceeded via rateLimit mock', async () => {
    ...
    const req = new NextRequest('http://localhost/api/aria/chat', { method: 'POST', body: JSON.stringify({ message: 'hi', subject: 'MATHEMATIQUES' }), headers: { 'Content-Type': 'application/json', 'x-forwarded-for': '1.1.1.1' } });
  ...
  it('401 when session missing studentId/parentId', async () => {
    ...
    const { POST } = require('@/app/api/aria/chat/route');
    const req = new NextRequest('http://localhost/api/aria/chat', { method: 'POST', body: JSON.stringify({ message: 'ok', subject: 'MATHEMATIQUES' }), headers: { 'Content-Type': 'application/json' } });
    const res = await POST(req as any);
  ...
  it('429 freemium usage reached', async () => {
    ...
    const fakePrisma = {
      student: {
        findUnique: jest.fn().mockResolvedValue({ id: 's1', freemiumUsage: { date: new Date().toISOString().split('T')[0], requestsToday: 5 } }),
        update: jest.fn(),
      }
    } as any;
  ...
  it('increments freemium usage when under quota (covers line 64)', async () => {
    ...
    const fakePrisma2 = {
      student: {
        findUnique: jest.fn().mockResolvedValue({ id: 's1', freemiumUsage: { date: new Date().toISOString().split('T')[0], requestsToday: 2 } }),
