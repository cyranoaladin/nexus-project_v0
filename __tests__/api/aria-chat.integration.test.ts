...
jest.mock('next-auth', () => ({
  getServerSession: jest.fn().mockResolvedValue({
    user: { id: 'user-student-1', role: 'ELEVE', firstName: 'Aria', lastName: 'Test', studentId: 's1', parentId: 'p1' },
  }),
}));
...
jest.mock('@/lib/aria/services', () => ({
  ...
  pdf_generator_service: { generate_pdf: jest.fn().mockResolvedValue({ url: 'http://example.com/doc.pdf' }) },
}));
...
describe('API /api/aria/chat Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    (prisma as any).student.findUnique = jest.fn().mockImplementation(async () => ({ id: studentId, freemiumUsage: freemiumState, user: { firstName: 'Aria', lastName: 'Test' }, parent: { user: { firstName: 'Parent', lastName: 'Test' } } }));
  ...
  it('Gating Freemium & Premium: freemium 429 after quota; premium 403 on non-subscribed subject', async () => {
    ...
    nextAuth.getServerSession.mockResolvedValueOnce({ user: { id: 'user-freemium', role: 'ELEVE', studentId: 's-free', parentId: 'p-free' } });
    // Mock student usage state
    let usage: any = { requestsToday: 5, date: new Date().toISOString().split('T')[0] };
    (prisma as any).student.findUnique = jest.fn().mockResolvedValueOnce({ id: 's-free', freemiumUsage: usage });
    const req429 = new NextRequest('http://localhost/api/aria/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: 'Req 6', subject: 'MATHEMATIQUES' }) });
    const res429 = await POST(req429);
    expect(res429.status).toBe(429);

    // Mock session: premium student Marie with Math+Physique subscription; call NSI -> 403
    nextAuth.getServerSession.mockResolvedValueOnce({ user: { id: 'user-premium', role: 'ELEVE', studentId: 's-prem', parentId: 'p-prem' } });
    (prisma as any).student.findUnique = jest.fn().mockResolvedValueOnce({ id: 's-prem' });
    (prisma as any).subscription.findMany = jest.fn().mockResolvedValueOnce([
    ...
    const req403 = new NextRequest('http://localhost/api/aria/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: 'Test NSI', subject: 'NSI' }) });
    ...
    require('@/lib/aria/orchestrator').AriaOrchestrator.prototype.handleQuery = jest.fn().mockImplementation(async function (this: any, _q: string, _s: any) {
      const subs = await (prisma as any).subscription.findMany({ where: { studentId: this['studentId'] } });
  ...
  it('Validation Mémoire Long Terme: prisma calls issued when building student context', async () => {
    const { POST } = require('@/app/api/aria/chat/route');
    const nextAuth = require('next-auth');
    nextAuth.getServerSession.mockResolvedValue({ user: { id: 'user-student-ctx', role: 'ELEVE', studentId: 's-ctx', parentId: 'p-ctx' } });
    (prisma as any).student.findUnique = jest.fn().mockResolvedValue({ id: 's-ctx', user: { firstName: 'Ctx', lastName: 'User' }, parent: { user: { firstName: 'P', lastName: 'U' }, id: 'p-ctx' } });
    ; (prisma as any).ariaMessage.findMany = jest.fn().mockResolvedValue([]);
    ; (prisma as any).assessment = { findMany: jest.fn().mockResolvedValue([]) };
    ; (prisma as any).mastery = { findMany: jest.fn().mockResolvedValue([{ concept: 'Probabilités', level: 'LOW', score: 0.4 }, { concept: 'Analyse', level: 'HIGH', score: 0.9 }]) };
    ; (prisma as any).document = { findMany: jest.fn().mockResolvedValue([{ id: 'doc1', title: 'Probabilités' }]) };
    ; (prisma as any).subscription = { findMany: jest.fn().mockResolvedValue([{ id: 'sub1', ariaSubjects: JSON.stringify(['MATHEMATIQUES']) }]) };
    const req = new NextRequest('http://localhost/api/aria/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: 'Bonjour', subject: 'MATHEMATIQUES' }) });
    await POST(req);
  ...
  it('Validation Personnalité Coach: system prompt conditionnel via decision_hints et statut', async () => {
    const { POST } = require('@/app/api/aria/chat/route');
    const nextAuth = require('next-auth');
    nextAuth.getServerSession.mockResolvedValue({ user: { id: 'user-student-ctx', role: 'ELEVE', studentId: 's-ctx2', parentId: 'p-ctx2' } });
    (prisma as any).student.findUnique = jest.fn().mockResolvedValue({ id: 's-ctx2', user: { firstName: 'Ctx', lastName: 'User' }, parent: { user: { firstName: 'P', lastName: 'U' }, id: 'p-ctx2' } });
    ; (prisma as any).ariaMessage.findMany = jest.fn().mockResolvedValue([]);
    ; (prisma as any).mastery = { findMany: jest.fn().mockResolvedValue([{ concept: 'Probabilités', level: 'LOW', score: 0.3 }]) };
    const services = require('@/lib/aria/services');
    (services.llm_service.generate_response as jest.Mock).mockImplementation(async (payload: any) => {
      expect(payload?.contexte_eleve?.decision_hints?.focusConcepts || []).toContain('Probabilités');
  ...
  it('Validation RAG Auto-Improvement: long response triggers ingest fetch', async () => {
    const { POST } = require('@/app/api/aria/chat/route');
    const nextAuth = require('next-auth');
    nextAuth.getServerSession.mockResolvedValue({ user: { id: 'user-student-rag', role: 'ELEVE', studentId: 's-rag', parentId: 'p-rag' } });
    (prisma as any).student.findUnique = jest.fn().mockResolvedValue({ id: 's-rag', user: { firstName: 'R', lastName: 'A' }, parent: { user: { firstName: 'P', lastName: 'U' }, id: 'p-rag' } });
    ; (prisma as any).ariaMessage.findMany = jest.fn().mockResolvedValue([]);
    const longText = '# Titre\n' + Array.from({ length: 180 }).map((_, i) => `mot${i}`).join(' ');
    jest.spyOn(global, 'fetch' as any).mockResolvedValueOnce({ ok: true, json: async () => ({ response: longText }) } as any);
    const services = require('@/lib/aria/services');
    jest.spyOn(services.llm_service, 'generate_response').mockResolvedValue({ response: longText } as any);
    const fetchSpy = jest.spyOn(global, 'fetch' as any);
    const req = new NextRequest('http://localhost/api/aria/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: 'Explique un long sujet', subject: 'MATHEMATIQUES' }) });
    await POST(req);
    expect(fetchSpy).toHaveBeenCalled();
    expect(fetchSpy.mock.calls.some((c: any[]) => String(c[0]).includes('rag_service') || String(c[0]).includes('/ingest'))).toBeTruthy();
