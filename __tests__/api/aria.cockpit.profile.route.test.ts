...
import { GET, PUT } from '@/app/api/aria/cockpit/profile/route';
...
jest.mock('@/lib/aria/cockpit/profile-service', () => {
  ...
  return {
    ...actual,
    getAriaCockpitProfile: jest.fn(),
    upsertAriaCockpitProfile: jest.fn(),
  };
});
...
/** Spécialité mathématiques, réellement suivie via le SSoT d'inscriptions. */
...
beforeEach(() => {
  jest.clearAllMocks();
  (prisma.studentAcademicEnrollment.findMany as jest.Mock).mockResolvedValue(STUDENT_ENROLLMENTS);
  (getAriaCockpitProfile as jest.Mock).mockResolvedValue(PROFILE);
  (upsertAriaCockpitProfile as jest.Mock).mockResolvedValue(PROFILE);
});
...
describe('GET /api/aria/profile', () => {
  ...
  it('retourne profil scolaire, profil ARIA et setupState', async () => {
    ...
    expect(body.ariaProfile.pinnedCourseKeys).toEqual(['maths-terminale-eds']);
