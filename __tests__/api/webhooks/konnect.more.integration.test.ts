...
describe('API /api/webhooks/konnect additional branches', () => {
  ...
  it('401 when signature missing or invalid', async () => {
    ...
    let res = await POST({ text: async () => '{}', headers: new Map(), url: 'http://localhost/api/webhooks/konnect' } as any);
  ...
  it('handles completed subscription and allocates credits', async () => {
    (prisma as any).payment.findUnique = jest.fn().mockResolvedValue({ id: 'p1', status: 'PENDING', type: 'SUBSCRIPTION', metadata: { studentId: 's1', itemKey: 'ACCÈS PLATEFORME' } });
    ...
    (prisma as any).subscription.findFirst = jest.fn().mockResolvedValue({ id: 'sub1', creditsPerMonth: 4 });
  ...
  it('handles failed status', async () => {
    (prisma as any).payment.findUnique = jest.fn().mockResolvedValue({ id: 'p1', status: 'PENDING', type: 'SUBSCRIPTION', metadata: {} });
    (prisma as any).payment.update = jest.fn().mockResolvedValue({});
    const { POST } = require('@/app/api/webhooks/konnect/route');
