...
describe('Konnect webhook additional status branches', () => {
  ...
  beforeAll(() => { process.env.KONNECT_WEBHOOK_SECRET = SECRET as any; });
  afterAll(() => { process.env.KONNECT_WEBHOOK_SECRET = originalEnv; });

  function signedRequest(body: any) {
    const raw = JSON.stringify(body);
    const sig = crypto.createHmac('sha256', SECRET).update(raw, 'utf8').digest('hex');
    return { text: async () => raw, headers: new Map([["x-konnect-signature", sig]]), url: 'http://localhost/api/webhooks/konnect' } as any;
  }

  beforeEach(() => {
    jest.clearAllMocks();
    (prisma as any).payment = (prisma as any).payment || {};
    (prisma as any).payment.findUnique = jest.fn().mockResolvedValue({ id: 'p1', status: 'PENDING', type: 'CREDIT_PACK', metadata: {} });
