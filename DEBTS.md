...
| Deux pipelines PDF (`tools/pdf-generator/generate_all_pdfs.py` et `scripts/pre-rentree/document_templates.py`) | P1 | Voir constat détaillé ci-dessous ; consolidation hors périmètre de cette PR (risque de casser la chaîne de gouvernance reproductible) |

**Décision direction (2026-07-25)** : garder les deux pipelines, dette confirmée **ouverte**, consolidation reportée post-go-live. L'ambiguïté de nommage « legacy » (signalée par l'audit) est résolue sans renommage — un commentaire d'en-tête explicite a été ajouté dans les deux fichiers (`tools/pdf-generator/generate_all_pdfs.py`, `scripts/pre-rentree/document_templates.py`) précisant lequel est production-publique et lequel est revue-gouvernance, option choisie comme la moins risqueuse (le renommage aurait touché `package.json`, un nom de fichier pytest et une archive historique figée).

**Constat sur les deux pipelines PDF (audit qualité 2026-07-25)** : `tools/pdf-generator/generate_all_pdfs.py` (`npm run pre-rentree:legacy-pdfs`) et `scripts/pre-rentree/document_templates.py` (via `generate_documents.py`, `npm run pre-rentree:build`) implémentent chacun, indépendamment, un jeu de documents quasi identique (guide parents, brochure, comparatif Fondations/Premium, planning, programme par niveau, tarifs) avec deux codes HTML/CSS séparés. **Ils ne sont pas une fuite de données** : les deux lisent en dernier ressort les mêmes sources canoniques (`data/campaigns/pre-rentree-2026.json`, `content/pre-rentree-2026/modules.json`, `data/pricing.canonical.json`), directement pour le premier, via le snapshot `.artifacts/pre-rentree-2026/publication.snapshot.json` pour le second — et les deux sont testés indépendamment (`test_legacy_pdf_generator_contract.py` / suite `pre-rentree:build`+`pre-rentree:audit`), tous verts. **Ils ont des rôles distincts** : le pipeline « legacy » produit les PDF réellement diffusés au public (`assets/campaigns/pre-rentree-2026/documents-final/` → `public/documents/pre-rentree-2026/`) ; le pipeline « build » produit le paquet de revue gouvernance (`.artifacts/pre-rentree-2026/build/`, manifeste lié par hash, workflow d'approbation propriétaire, `npm run pre-rentree:ci`). Le nom « legacy » est donc trompeur (c'est le pipeline actif de production, pas un résidu) — signalé pour clarification de nommage, pas pour suppression. **Décision de ne pas factoriser maintenant** : fusionner deux moteurs de rendu HTML/CSS indépendants et éprouvés dépasse le périmètre d'un correctif atomique et risquerait de casser la chaîne de reproductibilité/gouvernance qui est au cœur de la culture fail-closed du projet. Dette documentée, priorité P1, à traiter dans un chantier dédié avec tests de non-régression sur les deux chaînes avant toute fusion.
