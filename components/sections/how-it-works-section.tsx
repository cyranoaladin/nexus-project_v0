    ...
    <section id="methodologie" className="bg-surface-darker py-20">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="relative">
          <div className="absolute left-0 right-0 top-1/2 hidden h-px -translate-y-1/2 bg-brand-accent/30 lg:block" />
          <div className="grid gap-6 lg:grid-cols-4">
            {steps.map((step, index) => (
              ...
              >
                <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full border border-brand-accent/40 bg-black/30 text-lg font-semibold text-brand-accent">
                  {index + 1}
                </div>
                ...
                {index < steps.length - 1 ? (
                  <div className="absolute right-[-12px] top-1/2 hidden h-6 w-6 -translate-y-1/2 rotate-45 border-t border-r border-brand-accent/40 bg-surface-darker lg:block" />
                ) : null}
              </div>
            ))}
          </div>
