    ...
    >
      ...
      <div className={`border-b border-lux-line/40 px-6 ${featured ? 'pt-4' : 'pt-5'} pb-4`}>
        ...
        <p className="mt-1 text-sm text-lux-slate">
          {scenario.deposit > 0
            ? `Acompte ${fmtTND(scenario.deposit)} (non remboursable sauf non-ouverture du groupe) + ${scenario.months} mensualités · Total ${fmtTND(scenario.grandTotal)}`
            : `Sans acompte · ${scenario.months} mensualités · Total ${fmtTND(scenario.grandTotal)}`}
        </p>
