...
function getTargetChapter(
  ...
  if (weak.length > 0) {
    const { chapId } = weak[0];
    return { chapId, chapTitre: resolveChapTitle(chapId, programmeData) };
  }
 ...
 */
export const RAGFlashCard: React.FC<RAGFlashCardProps> = ({ onShowMore, store, programmeData }) => {
  ...
  useEffect(() => {
    ...
    const target = getTargetChapter(store.diagnosticResults, dueReviews, programmeData);
    setContext(target);
