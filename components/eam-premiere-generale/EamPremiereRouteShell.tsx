...
export function EamPremiereRouteShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!session || session.user.role !== "ELEVE") {
      router.push("/auth/signin?callbackUrl=/dashboard/eleve/eam-premiere");
    }
      ...
      <main className="min-h-screen bg-surface-darker text-neutral-100">
        <div className="flex min-h-screen items-center justify-center px-4">
          <div className="text-center">
            ...
            <p className="text-sm text-neutral-400">Chargement du sprint EAM...</p>
          </div>
        </div>
      </main>
    );
    ...
    <main className="min-h-screen overflow-x-hidden bg-surface-darker text-neutral-100">
      <div className="mx-auto w-full max-w-7xl px-3 py-4 sm:px-5 sm:py-6 lg:px-8">
        <div className="mb-4 flex min-w-0 flex-wrap items-center gap-2">
          ...
          <Link href="/dashboard/eleve/eam-premiere/livret">
