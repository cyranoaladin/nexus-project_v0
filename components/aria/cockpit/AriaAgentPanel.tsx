...
export function AriaAgentPanel({ cockpit, onOpenChat }: AriaAgentPanelProps) {
  ...
  return (
    <section id="aria-agent" aria-labelledby="aria-agent-title" className="space-y-4">
      ...
      {!cockpit.capabilities.conversationHistory ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Card className="border-white/10 bg-surface-card">
            <CardContent className="py-4">
              ...
              <p className="mt-1 text-2xl font-semibold text-neutral-100">{cockpit.aria.totalConversations}</p>
            </CardContent>
          </Card>
          <Card className="border-white/10 bg-surface-card">
            <CardContent className="py-4">
              <p className="text-xs text-neutral-400">Messages aujourd’hui</p>
              <p className="mt-1 text-2xl font-semibold text-neutral-100">{cockpit.aria.messagesToday}</p>
            </CardContent>
          </Card>
        </div>
      )}
