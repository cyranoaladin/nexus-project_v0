...
export default function CoachMathsStageListPage() {
  ...
  const fetchStudents = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/coach/maths-premiere-stage-printemps/students');
    ...
    <div className="mx-auto max-w-5xl space-y-8 px-4 py-10">
      ...
      <div>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-white">Bilan coach — Stage de printemps Mathématiques</h1>
          </div>
        </div>
        ...
        {(Object.entries(STATUS_CONFIG) as [BilanStatus, typeof STATUS_CONFIG[BilanStatus]][]).map(([key, cfg]) => (
          <button
            ...
            className={`rounded-2xl border p-4 text-left transition hover:bg-white/5 ${
              filter === key ? 'border-indigo-500/40 bg-indigo-500/10' : 'border-white/10 bg-white/5'
            }`}
          >
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-white/5 py-2 pl-9 pr-4 text-sm text-neutral-200 placeholder-neutral-600 focus:border-indigo-500/50 focus:outline-none sm:w-64"
          />
        </div>
      </div>
