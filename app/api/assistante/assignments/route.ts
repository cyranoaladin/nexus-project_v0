...
export async function GET(request: Request) {
  try {
    ...
    const statusResult = statusQuerySchema.safeParse(statusParam);
...
export async function POST(request: Request) {
  try {
    ...
    // The validation snapshot and every write share one serializable transaction.
    // The historical partial index alone only protects duplicates of the same type.
    return await prisma.$transaction(async (tx) => {
      const now = new Date();
      const startsAt = validated.startsAt ? new Date(validated.startsAt) : now;
      ...
      const enrollmentRows = await tx.studentAcademicEnrollment.findMany({
      ...
      // An ACTIVE authority must be ended explicitly before replacing it, including
      // future/expired rows and assignments of another type.
      const existingAssignments = await tx.coachStudentAssignment.findMany({
        where: {
      ...
      const createdAssignments = await Promise.all(
        validated.studentIds.map((studentId) =>
          tx.coachStudentAssignment.create({
            data: {
              coachId: validated.coachId,
              studentId,
    ...
    // A concurrent mutation must be reviewed against fresh state, never retried silently.
    if (error instanceof Prisma.PrismaClientKnownRequestError && ['P2002', 'P2034'].includes(error.code)) {
      return NextResponse.json(
        {
          error: 'Conflict',
          message: 'Conflit d’assignation, rechargez les données avant de réessayer',
        },
        { status: 409 }
      );
    }
