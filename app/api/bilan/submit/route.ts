...
export async function POST(req: Request) {
  ...
  try {
    if (data.sendEmailToStudent && student.user?.email) {
      await sendBilanEmail(student.user.email, "Votre bilan Nexus Réussite", "<p>Veuillez trouver votre bilan en pièce jointe.</p>", pdfBuffer);
    }
    if (data.sendEmailToParent && student.parent) {
      const parentUser = await prisma.user.findUnique({ where: { id: student.parent.userId } });
      if (parentUser?.email) {
        await sendBilanEmail(parentUser.email, "Bilan Nexus Réussite (parent)", "<p>Veuillez trouver le bilan de votre enfant en pièce jointe.</p>", pdfBuffer);
      }
    }
  } catch (e) {
