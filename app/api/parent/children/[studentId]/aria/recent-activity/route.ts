...
export async function GET(
  ...
  try {
    ...
    if (!courseKey) {
      throw new AriaError('BAD_REQUEST', 400, 'Clé de cours manquante.', { reasonCode: 'ARIA_PARENT_RECENT_ACTIVITY_COURSE_KEY_MISSING' });
    }

    const activity = await listAriaRecentActivityForParent({
      actor: { userId: session.user.id, role: session.user.role },
      studentId,
      courseKey,
    });
    return NextResponse.json({ studentId, courseKey, activity });
  } catch (error) {
