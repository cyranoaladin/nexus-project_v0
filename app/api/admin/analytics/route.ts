...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalRevenue = formattedRevenueData.reduce(
      (sum: number, item: any) => sum + item.amount,
      0
    );
    const totalUsers = formattedUserGrowthData.reduce(
      (sum: number, item: any) => sum + item.count,
      0
    );
    const totalSessions = formattedSessionData.reduce(
      (sum: number, item: any) => sum + item.count,
      0
    );
    const totalSubscriptions = formattedSubscriptionData.reduce(
      (sum: number, item: any) => sum + item.count,
      0
    );
