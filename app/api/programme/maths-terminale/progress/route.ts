...
export async function POST(request: Request) {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    // Graceful fallback path: client keeps localStorage as source of truth.
    return NextResponse.json({ ok: true, persisted: false, mode: 'local-only' }, { status: 200 });
  }
  ...
  try {
    ...
    if (error) {
      return NextResponse.json({ ok: false, persisted: false, error: error.message }, { status: 500 });
    }

    return NextResponse.json({ ok: true, persisted: true }, { status: 200 });
  } catch {
    return NextResponse.json({ ok: false, persisted: false, error: 'Failed to persist progress' }, { status: 500 });
  }
}
...
export async function GET() {
  ...
  const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!supabaseUrl || !serviceRoleKey) {
    return NextResponse.json({ ok: true, data: null, mode: 'local-only' }, { status: 200 });
  }
