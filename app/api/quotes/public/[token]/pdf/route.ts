 ...
 * a candidat-individuel quote that isn't CARTE_VALIDATED_DEFINITIVE yet
 * (collectQuoteEmissionBlockers non-empty) already returns "not found" from
 * that function — this route inherits that block automatically, it does
 * not re-implement it.
 *
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  const studentName = student?.user
    ? [student.user.firstName, student.user.lastName].filter(Boolean).join(' ') || 'Non renseigné'
    : 'Non renseigné';
