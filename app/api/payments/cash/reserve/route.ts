...
export async function POST(req: NextRequest) {
  ...
  const rec = await (prisma as any).paymentRecord.create({ data: { provider: 'cash', externalId: 'cash-reservation', userId, packId: Number(packId), amountTnd: Number(amountTnd), currency: 'TND', status: 'pending' } });
  try {
