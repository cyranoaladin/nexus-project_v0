/**
 * DiagnosticDefinition — NSI Première Spécialité, Pallier 2
 * Version: v1.3 — Domains loaded from compiled JSON (CdC §3.3)
 */

import type { DiagnosticDefinition } from '../types';
import compiledDomains from './generated/nsi-premiere-p2.domains.json';
...
export const NSI_PREMIERE_P2: DiagnosticDefinition = {
  key: 'nsi-premiere-p2',
  version: 'v1.3',
  ...
  scoringPolicy: {
    ...
    thresholds: {
      ...
      conditional: { readiness: 42, risk: 72 },
    },
  },

  prompts: {
    version: 'v1.0',
    eleve: `Tu es un expert pédagogique bienveillant en NSI (Première spécialité, programme français).
...
- Si du contexte pédagogique RAG est fourni, le citer.
- Si une donnée est manquante ou "not_studied", dire "à planifier" et non "faiblesse".
- Focus NSI 1ère : représentation des données, algorithmique de base, Python, architecture.

## Structure obligatoire
1. **Résumé 15 secondes** : 3 puces (force, priorité, objectif 2 semaines)
2. **Score & lecture** : explication simple du niveau
3. **Tes 3 priorités** : avec micro-exercice concret (code Python, schéma binaire, etc.)
...
~400 mots. Retourne UNIQUEMENT le Markdown.`,
    ...
    parents: `Tu es un expert pédagogique professionnel en NSI (Première spécialité, programme français).
Tu travailles pour Nexus Réussite, centre de soutien scolaire en Tunisie.
...
- Expliquer la NSI de manière accessible (informatique ≠ bureautique).
- "not_studied" = "sera abordé prochainement", pas une faiblesse.

## Structure obligatoire
1. **Synthèse** : où en est l'élève à mi-année
2. **Risques** : compréhension du code, logique, gestion du temps
3. **Plan réaliste maison** : 15-20 min/jour de pratique Python
...
~500 mots. Retourne UNIQUEMENT le Markdown.`,
    ...
    nexus: `Tu es un expert pédagogique technique en NSI (Première spécialité, programme français).
Tu travailles pour Nexus Réussite, centre de soutien scolaire en Tunisie.
...
- Focus NSI : compréhension du code > mémorisation, débuggage, logique booléenne.
...
~600 mots. Tableaux markdown. Retourne UNIQUEMENT le Markdown.`,
  ...
  ragPolicy: {
    collections: ['ressources_pedagogiques_nsi_premiere'],
    maxQueries: 4,
    topK: 2,
  },
};
