 ...
 *
 * Query budget (≤ 12):
 *   Q1  : student + inlined relations (sequential, gates everything)
 *   Q2  : MathsProgress for current track
 *   Q3  : recent bilans (audience=nexus filtered server-side)
 *   Q4  : stage reservations
 *   Q5  : user documents (extended Lot B: includes uploadedBy.role)
 *   Q6  : entitlements
 *   Q7  : active trajectory
 *   Q8  : next step engine
 *   Q9  : stage bilans (conditional — only if reservations exist)
 *   Q10 : invoices (Lot B — addressed to student via beneficiaryUserId)
 *
 * Filtering rules:
...
function userDocCategory(
  doc: { uploadedById: string | null; uploadedBy: { id: string; role: UserRole } | null },
...
export async function buildStudentDashboardPayload(userId: string): Promise<EleveDashboardData> {
  ...
  const [
    ...
    // Q5: User documents — extended for Hub Ressources (Lot B):
    //   include uploaderId/role/firstName/lastName so we can distinguish
    //   USER_DOCUMENT (uploaded by self / system / admin) from
    //   COACH_RESOURCE (uploaded by a coach for the student).
  ...
  // Hub Ressources Pédagogiques (Lot B) — uses already-fetched data only.
  const hub = buildHub({
    level: gradeLevel,
    track: academicTrack,
    ...
    stageItems: [...upcomingStages, ...pastStages],
  });
