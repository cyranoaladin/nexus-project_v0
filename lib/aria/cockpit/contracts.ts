...
/** Profil pédagogique ARIA (modèle additif `aria_cockpit_profiles`). */
export interface AriaCockpitProfileDTO {
...
// Ces quatre types sont la projection COCKPIT (plate, avec compteurs et
// back-refs domainId) des graphes de compétences. Ils sont distincts, par
// conception, du type interne du même nom dans
// `lib/aria/curriculum/skill-graph.ts` (imbriqué, sans compteur ni version) :
// ce dernier est le point d'accès canonique, déjà utilisé par le pipeline de
// prompt du moteur de conversation, et n'est jamais modifié ici. L'adaptateur
// `lib/aria/cockpit/skill-views.ts` fait le pont entre les deux.
...
export interface AriaSkillGraph {
  ...
  /** `null` : le module canonique n'expose pas de version de schéma à ce niveau. */
...
export interface AriaCockpitDTO {
  ...
  readonly profile: AriaCockpitProfileDTO;
