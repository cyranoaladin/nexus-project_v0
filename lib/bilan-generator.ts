...
export async function generateBilans(
  ...
  try {
    ...
    console.warn('[F30] RAG search failed, proceeding without pedagogical context:', error);
...
${data.examPrep.mainRisk ? `- Risque principal : "${data.examPrep.mainRisk}"` : ''}`;
  ...
  return { eleve, parents, nexus, ragUsed: false, ragHitCount: 0, ragCollections: [], ragError: false };
}
