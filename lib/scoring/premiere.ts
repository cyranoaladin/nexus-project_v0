...
// Raw JSON structure as described in pipeline_bilan_premiere_maths.md
const RAW = {
  meta: {
...
function domainLabelPrem(d: string): string {
  switch (d) {
    case 'NombresCalculs': return 'Nombres & Calculs';
    case 'EquationsInequations': return 'Équations / Inéquations';
    case 'Fonctions': return 'Fonctions';
    case 'GeometrieTrig': return 'Géométrie & Trigonométrie';
    case 'ProbaStats': return 'Probabilités & Statistiques';
    case 'AlgoLogique': return 'Algorithmique & Logique';
    default: return d;
  }
}

export const QCM_PREMIERE_QUESTIONS: QcmQuestion[] = RAW.questions.map((q) => ({
  id: q.id,
  domain: domainLabelPrem(q.domain),
  weight: q.weight,
  prompt: q.statement,
  type: "single",
