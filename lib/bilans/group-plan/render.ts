...
export function renderGroupPlanHtml(plan: GroupPlan, suppliedIdentity: RenderIdentity): string {
  ...
  // Below, the calibration range uses '›' (not '→'): neither embedded
  // font contains the arrow glyph, so Chromium fell back to a system font for
  // it -- different per machine, breaking the byte-exact fixture comparison
  // in __tests__/bilans/group-plan-render.test.ts outside whichever
  // environment last regenerated the fixture. '›' is covered by DM Sans.
