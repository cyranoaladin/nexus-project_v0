...
test.describe('Admin RAG Upload', () => {
  test('parse metadata and ingest document', async ({ page, browserName, context }) => {
    ...
    try {
      await page.waitForURL('**/dashboard/admin/rag-management', { timeout: 15000 });
    } catch {}
    ...
    await page.waitForSelector('[data-testid="rag-file-upload"]', {
      state: 'attached',
      timeout: 30000,
    });
    ...
    await page.route('**/api/admin/rag-ingest', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({ id: 'doc-xyz' }),
      });
    });
