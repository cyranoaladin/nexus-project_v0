...
test.describe('ARIA Interaction Flow', () => {
  ...
  test('should allow authenticated users to ask a question', async ({ page }) => {
    ...
    let inputForMessage = page.locator('[data-testid="aria-input"], [data-testid-aria="aria-input"], [data-testid="nexus-aria-input"], input[placeholder="Posez votre question à ARIA..."]').first();
    try {
      ...
      inputForMessage = page.locator('[data-testid="aria-input"], [data-testid-aria="aria-input"], [data-testid="nexus-aria-input"], input[placeholder*="Posez"]').first();
      await expect(inputForMessage).toBeVisible({ timeout: 20000 });
    }
    await inputForMessage.fill('Bonjour, peux-tu m\'expliquer les dérivées ?');
    const sendButton = page.locator('[data-testid="aria-send"], button[aria-label="Envoyer le message"]').first();
    try {
