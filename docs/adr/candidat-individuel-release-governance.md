...
- Protection de branche formellement équivalente : si l'API GitHub de protection classique répond exactement `Branch protection has been disabled (HTTP 404)`, la qualification exige à la place un ruleset de branche actif couvrant uniquement `refs/heads/release/candidat-individuel-prod-final`, sans exclusion ni bypass (`current_user_can_bypass=never`). Il doit interdire suppression et non-fast-forward, imposer l'historique linéaire et les deux checks exacts `CI Success` et `Hermetic DB Order Matrix` avec politique stricte. Toute autre erreur de l'API classique reste bloquante. La preuve enregistre le mécanisme, l'identifiant du ruleset et sa couverture effective; elle est réinterrogée avant attestation et cutover.
