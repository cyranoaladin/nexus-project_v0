...
- Payload forgé : le reçu externe contient les realpaths et digests du vrai `.next/standalone`, du staging, de la provenance embarquée et de l'entrée de build. Copier ou réécrire l'un de ces éléments rompt la liaison.
...
- CI homonyme ou périmée : les pages complètes de workflow-runs et check-runs sont interrogées. Le dernier run réussi de `.github/workflows/ci.yml` à la SHA exacte est retenu; chaque check obligatoire doit appartenir à l'application GitHub Actions officielle et son URL doit viser ce run.
...
- Archive hostile/TOCTOU : le vérificateur ouvre sans suivre de symlink, crée une copie temporaire privée immuable, audite ses en-têtes avant extraction puis extrait et hache cette même copie. Traversées, chemins absolus, liens dangereux, types spéciaux, doublons, métadonnées/PAX non canoniques et limites de taille ou de membres sont fail-closed.
