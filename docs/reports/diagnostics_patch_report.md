...
## AUDIT §8 — Preuve `/` exclu du build

### 1. `tsconfig.json` — `` est inclus dans le type-check mais PAS dans le build
...
### 2. `next.config.mjs` — Aucune référence à ``

Le fichier `next.config.mjs` ne contient aucune inclusion de ``. Next.js standalone build (`output: 'standalone'`) ne trace que les fichiers importés depuis les entry points (`app/`, `pages/`).

### 3. Aucun import de `` dans le code applicatif

```bash
grep -r "from.*\\|import.*\\|require.*\" lib/ app/ components/
# Result: 0 matches
```

### 4. `jest.config.unit.js` — `` tests isolés

```javascript
testMatch: [
  // ... app tests ...
  '**//core/**/*.test.(js|ts|tsx)',  //  tests run separately
],
...
**Verdict** : ✅ `/` est complètement isolé du build Next.js. Ses erreurs TS n'affectent pas l'application.
...
| Worker force exit warning | Low | Timers , n'affecte pas les résultats |
...
| `/` exclu du build | ✅ | 0 imports depuis app/, non tracé par Next.js standalone |
