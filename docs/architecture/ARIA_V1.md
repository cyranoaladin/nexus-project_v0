...
- strictement projetée depuis le Turn par un trigger DB pour les nouveaux messages liés ;
...
Le contenu d'une ResourceVersion est lu derrière le port applicatif. Sous Linux, le repository ouvre depuis le file descriptor de la racine allowlistée avec `openat2(RESOLVE_BENEATH|RESOLVE_NO_SYMLINKS)` et `O_NOFOLLOW`; si cette primitive atomique n'est pas disponible, il échoue fermé, sans fallback check-then-open. `fstat`, taille, SHA-256, détection MIME et streaming utilisent tous le **même file descriptor**, ensuite fermé dans un `finally`, puis la réponse applique un `Content-Disposition` sûr. Aucun chemin local n'entre dans une erreur publique.
