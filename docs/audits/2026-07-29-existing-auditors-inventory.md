...
|---|---|---|---|
| `scripts/pre-rentree/final-public-release-audit.mjs` (`--source`) | Fuite interne + faits commerciaux dupliqués, pré-rentrée uniquement | **Oui** — en CI (test Jest, bloquant) | — |
| `scripts/pre-rentree/final-public-release-audit.mjs` (`--artifacts`) | Fuite interne dans le bundle JS applicatif | **Oui** — job CI non bloquant (`public-content-audit`) | **Valeur détective réelle nulle à ce jour** (0 vrai positif sur toute cette mission, 7 findings tous du bruit vendor/framework identifié). Conservé pour sa valeur défensive (coût de maintenance faible, 5 exceptions datées et justifiées). **Réexamen prévu : 2026-10-27 (+90 jours)** — si aucun vrai positif d'ici là, à retirer. |
| `scripts/pre-rentree/final-public-release-audit.mjs` (`--rendered`) | Fuite interne dans le HTML rendu | **Oui** — job CI non bloquant, alimenté par `--save-html` | 1 vrai positif trouvé dès la première exécution avec une entrée réelle (`pre2026-pack-`) |
...
| `scripts/marketing/public-content-audit.mjs` — mode inversé (`orphan-payload-string`, ajouté 2026-07-29) | Détecte tout champ de charge d'hydratation jamais affiché, sans connaître son nom à l'avance — a retrouvé `rationale` sans le nommer | **Non — couverture partielle, outil de diagnostic MANUEL uniquement, ne pas s'y fier seul** | voir `docs/audits/2026-07-29-decision-1-orphan-payload-detection.md` : 759 findings résiduels après 3 corrections, dominés par du contenu interactif légitime (catalogues/assistants à divulgation progressive) qu'un fetch statique ne peut pas distinguer d'une fuite ; ne détecte que les valeurs de type chaîne, pas les tableaux (`pack_product_ids` non couvert par ce mode) |
