...
- **Constat :** Après 3 correctifs ciblés (fetchPriority hero, GA lazyOnload, hero intro élargi), les gains sont : / +15 perf (68→83), /offres +19 perf (67→86), FCP divisé par 2 (3.0→1.5s, 3.2→1.4s), CLS = 0 préservé. Le reliquat (LCP 3.3-4.3s) est le coût d'hydratation de 103 KB de React+Next.js runtime irréductible sous CPU throttling 4×. 0 import lourd, 0 bundle superflu, 0 framer-motion, 0 dashboard dans les bundles publics.
...
- **Hypothèse (à confirmer) :** la simulation Lighthouse applique un CPU 4× ralenti ; les Core Web Vitals terrain (CrUX / PageSpeed Insights field data) sont attendus meilleurs, mais ne seront mesurables qu'une fois le trafic accumulé. À vérifier dans PageSpeed Insights (données réelles) ~28 jours après mise en ligne.
...
- **Sévérité :** Faible — le plancher est prouvé structurel, la validation terrain reste à faire
