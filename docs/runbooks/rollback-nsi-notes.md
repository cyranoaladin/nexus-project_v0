...
- Les scripts temporaires de provisioning (`check-nsi-users.ts`, `prod-upsert-nsi-users.ts`) ont été déplacés hors workspace Git vers `/root/nexus-maintenance-scripts/` ; ils ne doivent pas être commités ni laissés dans `/var/www/nexus-project_v0/scripts/` s'ils ne sont pas versionnés.
