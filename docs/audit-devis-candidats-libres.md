...
**D4 — Échéancier candidat-libre de référence. RÉVISÉ (2026-08-24, deuxième révision), remplace les deux décisions précédentes (2026-08-23 : 10%+8 ; première révision du 2026-08-24 : 25%+10).** **20% d'acompte + 9 mensualités** devient la référence par défaut. Corrige le modèle `deposit: 0` + 10 mensualités actuellement utilisé par les 6 SKU candidat-libre et par `lib/quotes/pricing.ts` sur cette branche.

Fait vérifié qui motive la révision : la production (`main`) n'a jamais eu de modèle sans acompte pour le candidat libre. `term-libre-mixte` (catalogue `main`, remplacé par les 6 SKU actuels sur cette branche — cf. `git show main:data/pricing.canonical.json`) applique déjà le modèle générique 30% + 9 mensualités : `deposit: 2370` sur `price_annual: 7900` (= 30% exactement), `n_installments: 9`. Le passage à `deposit: 0` sur cette branche n'est donc pas une décision produit délibérée retrouvée dans l'historique, mais une régression de protection non actée — l'acompte non remboursable est le seul garde-fou contre le no-show sur une population non scolarisée et sans ancrage institutionnel, alors que des heures enseignant y sont préengagées.

20% (ni 30%, ni 25%, ni les 10% de la toute première décision) est retenu comme compromis final : plus doux que le modèle générique sur les totaux annuels les plus élevés du catalogue candidat-libre (jusqu'à 16 900 TND pour `terminale-libre-integrale`), tout en restaurant une protection réelle. **9 mensualités (pas 10)** — ce compte s'aligne sur le `N` par défaut du modèle générique (`computeSchedule()`, §1.5), donc la migration en Lot 5 réutilise l'infrastructure existante avec un seul paramètre modifié (le taux d'acompte), sans introduire de `N` distinct pour le candidat libre. Migration des 6 SKU et de `lib/quotes/pricing.ts` à prévoir en Lot 5, sans impact rétroactif sur les `Quote` déjà émis (figés par `snapshotRegles`). La variante `JALONS_EPREUVES` du brief §6.7 reste disponible en option, non par défaut ; les références 10%+8 et 25%+10 ne sont pas conservées comme variantes (aucun cas d'usage identifié qui les justifierait séparément de 20%+9).
...
Décisions **D1 à D6 tranchées** le 2026-08-23 (§5), **D4 révisé deux fois le 2026-08-24** (référence finale : **20% + 9 mensualités**, cf. §5). Plan d'implémentation détaillé du Lot 1 (socle réglementaire étendu dans `lib/exams/`) écrit et découpé en étapes de 2-5 minutes : `docs/superpowers/plans/2026-08-23-lot1-socle-reglementaire.md`. Aucune tâche de ce plan ne dépend de D4 (D4 relève du Lot 5, tarification) — prêt à exécuter en TDD dès validation.
