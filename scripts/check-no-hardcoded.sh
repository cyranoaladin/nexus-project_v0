...
if [ "$COUNT" -gt 0 ]; then
  echo "FAIL: $COUNT hardcoded matricule fiscal outside lib/legal.ts"
  FAIL=1
fi
...
if [ "$COUNT" -gt 0 ]; then
  echo "FAIL: $COUNT hardcoded email outside lib/legal.ts"
  FAIL=1
fi
...
if [ "$COUNT" -gt 0 ]; then
  echo "FAIL: $COUNT hardcoded phone outside canonical sources"
  FAIL=1
fi
...
if [ "$COUNT" -gt 0 ]; then
  echo "FAIL: $COUNT hardcoded entity name outside lib/legal.ts"
  FAIL=1
fi
