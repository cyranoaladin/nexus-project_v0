...
# -*- coding: utf-8 -*-
"""
Scoring + Radar (Terminale) — Nexus Réussite
Usage:
  python scoring_radar_terminale.py \
    --qcm data/qcm_premiere_for_terminale.json \
    --answers answers.json \
    --out results.json \
    --png radar.png

answers.json: {"Q1": 1, "Q2": 0, ...} (index d'option correcte)
"""
...
def is_correct(q, ans):
    correct = set(q['correct']) if isinstance(q.get('correct'), list) else set([q.get('answer')])
    try:
