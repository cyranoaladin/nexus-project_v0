...
test('crawl buttons & links on all pages', async ({ page, context }) => {
  ...
  for (const route of routes) {
    ...
    const clickables = p.locator('a:visible, button:visible, [role="button"]:visible, [data-testid]:visible');
    ...
    for (let i = 0; i < count; i++) {
      ...
      const status = await target.evaluate(() => performance.getEntriesByType('resource').length ? 200 : 200);
