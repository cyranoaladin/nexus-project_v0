...
async function main() {
  ...
  if (anyCoachProfile && anyStudent) {
    ...
    await prisma.session.createMany({
      data: [
        { studentId: anyStudent.id, coachId: anyCoachProfile.id, type: 'COURS_ONLINE', subject: 'MATHEMATIQUES', title: 'Révisions dérivées', scheduledAt, duration: 60, creditCost: 1, status: 'COMPLETED' },
        { studentId: anyStudent.id, coachId: anyCoachProfile.id, type: 'COURS_ONLINE', subject: 'ANGLAIS', title: 'Grammar', scheduledAt: now, duration: 60, creditCost: 1, status: 'SCHEDULED' },
      ]
    });
  }
...
async function extendedSeeds() {
  ...
  const badges = await prisma.badge.createMany({
    data: [
      { name: 'ASSIDUITE_BRONZE', description: '3 sessions suivies', category: 'ASSIDUITE', icon: '🥉', condition: '3_sessions' },
      { name: 'ASSIDUITE_ARGENT', description: '6 sessions suivies', category: 'ASSIDUITE', icon: '🥈', condition: '6_sessions' },
      { name: 'PROGRESSION_1', description: 'Amélioration notable', category: 'PROGRESSION', icon: '📈', condition: 'progress_1' },
      { name: 'CURIOSITE', description: 'Participe activement', category: 'CURIOSITE', icon: '🧠', condition: 'curiosity' },
    ],
    skipDuplicates: true,
  });

  // Ensure we have at least one coach per Subject (already ensured above), add availability for first three coaches (robust)
  let someCoaches = await prisma.user.findMany({ where: { role: 'COACH' }, take: 3, include: { coachProfile: true } as any });
  if (!someCoaches.length) {
    const fallbackCoach = await prisma.user.create({ data: { email: 'coach.fallback@nexus.com', password: await bcrypt.hash('password123', 12), role: 'COACH', firstName: 'Coach', lastName: 'Fallback' } });
    const fallbackProfile = await prisma.coachProfile.create({ data: { userId: fallbackCoach.id, pseudonym: 'Fallback', subjects: JSON.stringify(['MATHEMATIQUES']) } });
  ...
  const parentVar = await prisma.user.create({
    data: { email: 'parent.variations@nexus.com', password: await bcrypt.hash('password123', 12), role: 'PARENT', firstName: 'Parent', lastName: 'Variations' }
  });
  ...
  const childAUser = await prisma.user.create({ data: { email: 'enfantA@nexus.com', password: await bcrypt.hash('password123', 12), role: 'ELEVE', firstName: 'A', lastName: 'Var' } });
  const childA = await prisma.student.create({
    data: {
      ...
      credits: 10,
      subscriptions: {
        create: { planName: 'IMMERSION', monthlyPrice: 750, creditsPerMonth: 8, status: 'ACTIVE', startDate: new Date(), ariaSubjects: JSON.stringify(['MATHEMATIQUES', 'NSI']), ariaCost: 60 }
      }
    }
  });

  // Child B: CANCELLED hybride + zero credits
  const childBUser = await prisma.user.create({ data: { email: 'enfantB@nexus.com', password: await bcrypt.hash('password123', 12), role: 'ELEVE', firstName: 'B', lastName: 'Var' } });
  const childB = await prisma.student.create({
    data: {
      ...
      credits: 0,
      subscriptions: {
        create: { planName: 'HYBRIDE', monthlyPrice: 450, creditsPerMonth: 8, status: 'CANCELLED', startDate: new Date(Date.now() - 60 * 24 * 3600 * 1000), endDate: new Date(Date.now() - 30 * 24 * 3600 * 1000) }
      }
    }
  });

  // Child C: EXPIRED accès plateforme + ARIA one subject + subscription request pending
  const childCUser = await prisma.user.create({ data: { email: 'enfantC@nexus.com', password: await bcrypt.hash('password123', 12), role: 'ELEVE', firstName: 'C', lastName: 'Var' } });
  const childC = await prisma.student.create({
    data: {
      ...
      credits: 2,
      subscriptions: {
        create: { planName: 'ACCES_PLATEFORME', monthlyPrice: 120, creditsPerMonth: 4, status: 'EXPIRED', startDate: new Date(Date.now() - 60 * 24 * 3600 * 1000), endDate: new Date(Date.now() - 1 * 24 * 3600 * 1000), ariaSubjects: JSON.stringify(['ANGLAIS']), ariaCost: 30 }
      }
    }
  });
  await prisma.subscriptionRequest.create({
    data: { studentId: childC.id, requestType: 'PLAN_CHANGE', monthlyPrice: 250, reason: 'Upgrade', status: 'PENDING', requestedBy: parentVar.id, requestedByEmail: parentVar.email }
  });

  // Freemium usage edge-cases
  await prisma.student.update({ where: { id: childA.id }, data: { freemiumUsage: { requestsToday: 5, date: new Date().toISOString().split('T')[0] } as any } });
  await prisma.student.update({ where: { id: childB.id }, data: { freemiumUsage: { requestsToday: 0, date: new Date().toISOString().split('T')[0] } as any } });

  // Payments covering all statuses/types
  await prisma.payment.createMany({
    data: [
      { userId: parentVar.id, type: 'SUBSCRIPTION', amount: 250, description: 'Hybride mois en cours', status: 'PENDING', method: 'konnect' },
      { userId: parentVar.id, type: 'CREDIT_PACK', amount: 100, description: 'Pack crédits', status: 'REFUNDED', method: 'manual' },
      { userId: parentVar.id, type: 'SPECIAL_PACK', amount: 150, description: 'Pack spécial', status: 'COMPLETED', method: 'manual' },
    ]
  });
  ...
  const makeBooking = async (studentId: string, status: any, type: any, modality: any, offsetDays: number) => {
    ...
    await prisma.sessionReminder.create({ data: { sessionId: sb.id, reminderType: 'ONE_DAY_BEFORE', scheduledFor: new Date(sb.scheduledDate.getTime() - 24 * 3600 * 1000) } });
    await prisma.sessionNotification.create({ data: { sessionId: sb.id, userId: parentVar.id, type: 'SESSION_BOOKED', title: 'Session réservée', message: 'Votre session est réservée', method: 'EMAIL', status: 'SENT', sentAt: new Date() } });
  };
  ...
  await prisma.notification.createMany({
    data: [
      { userId: parentVar.id, userRole: 'PARENT', type: 'SUBSCRIPTION_REQUEST', title: 'Demande en attente', message: 'Votre demande de changement est en attente.', data: '{}' },
      { userId: childAUser.id, userRole: 'ELEVE', type: 'CREDIT_REQUEST', title: 'Crédits bas', message: 'Pense à recharger tes crédits.', data: '{}' },
    ]
  });
  ...
  const conv = await prisma.ariaConversation.create({ data: { studentId: childA.id, subject: 'ANGLAIS', title: 'Vocabulaire' } });
  await prisma.ariaMessage.createMany({ data: [
    { conversationId: conv.id, role: 'USER', content: 'Peux-tu m\'expliquer les phrasal verbs ?', feedback: true },
    { conversationId: conv.id, role: 'ASSISTANT', content: 'Bien sûr, commençons par...', feedback: false },
  ] });
  ...
  await prisma.creditTransaction.createMany({ data: [
    ...
    { studentId: childB.id, type: 'EXPIRATION', amount: -2, description: 'Expiration crédits', expiresAt: new Date() },
    { studentId: childC.id, type: 'REFUND', amount: 1, description: 'Remboursement' },
  ] });

  // Additional pedagogical content across subjects
  await prisma.pedagogicalContent.createMany({ data: [
    { title: 'Analyse syntaxique en Français', content: '# Grammaire...', subject: 'FRANCAIS', grade: 'Première', tags: '["grammaire"]', embedding: '[]' },
    { title: 'Guerres mondiales', content: '# Histoire...', subject: 'HISTOIRE_GEO', grade: 'Terminale', tags: '["20e_siecle"]', embedding: '[]' },
    { title: 'Photosynthèse', content: '# SVT...', subject: 'SVT', grade: 'Seconde', tags: '["biologie"]', embedding: '[]' },
  ] });
}
