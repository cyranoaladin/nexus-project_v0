...
- **Configuration (no defaults, fail-closed)**: `CORE_V2_ORGANIZATION_TIMEZONE` (IANA zone captured on each PlanningSeries), `CORE_V2_INVITATION_TTL_HOURS`, `CORE_V2_PASSWORD_RESET_TTL_MINUTES` and `CORE_V2_AUTH_MODE` (V1_ONLY | HYBRID | V2_ONLY, the auth rollout switch; HYBRID/V2_ONLY fail closed without a verified Core v2 database) — see `lib/core-v2/config.ts`. Academic-year dates are configured input at creation, never derived from a built-in calendar.
