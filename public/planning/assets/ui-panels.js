...
(function (global) {
  ...
  function renderPlanningHead(container, app) {
    ...
    container.appendChild(h('div', null, [
      h('h2', null, titles[view] || 'Planning'),
      h('div', { class: 'sub hint' }, subs[view] || ''),
  ...
  function openConfig(app, tab, editingId) {
    ...
    function rerender() {
      ...
      clear(body);
      const renderers = { teachers: renderTeachersConfig, rooms: renderRoomsConfig, subjects: renderSubjectsConfig, groups: renderGroupsConfig, settings: renderSettingsConfig };
  ...
  function renderTeachersConfig(body, app) {
    ...
    body.appendChild(h('div', { class: 'panel-title' }, [
      ...
      h('button', { type: 'button', class: 'btn primary sm', onclick: () => { app.commit('Ajout d\'un enseignant', (d) => { const t = Nexus.newTeacher(d); d.teachers.push(t); config.editingId = t.id; }); } }, '+ Ajouter')
    ]));
  ...
  function renderRoomsConfig(body, app) {
    ...
    body.appendChild(h('div', { class: 'panel-title' }, [
      ...
      h('button', { type: 'button', class: 'btn primary sm', onclick: () => app.commit('Ajout d\'une salle', (d) => { const r = Nexus.newRoom(d); d.rooms.push(r); config.editingId = r.id; }) }, '+ Ajouter')
    ]));
    const list = h('div', { class: 'manage-list' });
  ...
  function renderSubjectsConfig(body, app) {
    ...
    body.appendChild(h('div', { class: 'panel-title' }, [
      ...
      h('button', { type: 'button', class: 'btn primary sm', onclick: () => app.commit('Ajout d\'une matière', (d) => { const s = Nexus.newSubject(d); d.subjects.push(s); config.editingId = s.id; }) }, '+ Ajouter')
    ]));
    const list = h('div', { class: 'manage-list' });
  ...
  function renderGroupsConfig(body, app) {
    ...
    body.appendChild(h('div', { class: 'panel-title' }, [
      ...
      h('button', { type: 'button', class: 'btn primary sm', onclick: () => app.commit('Ajout d\'un groupe', (d) => { const g = Nexus.newGroup(d); d.groups.push(g); config.editingId = g.id; }) }, '+ Ajouter')
    ]));
    const list = h('div', { class: 'manage-list' });
  ...
  function renderSettingsConfig(body, app) {
    ...
    body.appendChild(h('div', { class: 'stack' }, [
      ...
      h('p', { class: 'help' }, 'Les modifications sont enregistrées automatiquement dans ce navigateur (' + (Nexus.storageAvailable() ? 'stockage local disponible' : 'stockage local indisponible') + '). Exportez régulièrement un fichier JSON comme sauvegarde ou pour transférer le planning vers un autre poste.')
    ]));
  }
